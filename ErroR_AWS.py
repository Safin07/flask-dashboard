...
machine_ids = [
    "68:B6:B3:54:E8:B2", "68:B6:B3:52:92:BE", "68:B6:B3:54:E8:32", "68:B6:B3:53:04:2E",
...
sign_in_payload = {"email": "smartmattress2023@gmail.com", "password": "Welcome@123"}
...
RECEIVER_EMAIL = os.getenv(
    'RECEIVER_EMAIL',
    'safin.chowdhury@variowell-development.com, jimesh.pitroda@variowell-development.com, '
    'maximilian.sommer@variowell.com, ansgar.buescher@variowell.com'
)
RECEIVER_EMAIL_LIST = [email.strip() for email in RECEIVER_EMAIL.split(',')]
PASSWORD = os.getenv('EMAIL_PASSWORD', 'Saf#Mail535!')
...
for machine_id in machine_ids:
    ...
    machine_error_payload = {
        "machineId": machine_id, "page": 1, "nFilter": {}, "sortBy": "DESC",
        "sortValue": "arrivalTime", "download": 0,
        "fields": ["machineId", "arrivalTime", "zoneValue", "error", "logCounter", "timeStamp"]
    }
    error_headers = {"Content-Type": "application/json", "x-access-token": access_token}
        ...
        for item in error_data.get('result', []):
            ...
            if arrival_time:
                ...
                if error_date == today_date:
                    ...
                    timestamp_str = datetime.fromtimestamp(timestamp_value, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z') if timestamp_value else "N/A"
                    error_message = (
                        f"Error on {error_date}: Code {error_code} - {error_name}, "
                        f"Details: Machine ID: {item.get('machineId')}, TimeStamp: {timestamp_str}"
                    )
                    print(error_message)
                    error_messages.append(error_message)
                    error_found = True
    except Exception as e:
        print(f"Error fetching data for machine {machine_id}: {e}")
