...
# Function to fetch data for the specific machine ID
def fetch_machine_data(machine_id, access_token):
    ...
    while True:
        machine_data_payload = {
            ...
            "download": 0,
            "fields": [],
            "limit": 100  # Set a reasonable limit for each page
        }
        machine_headers = {
            "Content-Type": "application/json",
        ...
        try:
            response = requests.post(single_machine_data_url, json=machine_data_payload, headers=machine_headers, verify=False)
            response.raise_for_status()
            if response.status_code == 200:
                ...
                logging.error(f"Failed to fetch data for Machine ID {machine_id}. Status code: {response.status_code}")
                logging.error("Response: %s", response.json())
                break
        except requests.exceptions.RequestException as e:
            logging.error(f"An error occurred while fetching data for Machine ID {machine_id}: %s", e)
            break
        ...
        if isinstance(row, dict):  # Ensure the row is a dictionary
            ...
            for col, value in row.items():
                if col == 'timeStamp':  # Convert timeStamp to CEST
                    device_date, device_time_full, _ = convert_to_cest(value)
                    structured_row['Device local Date'] = device_date
                    structured_row['Device local Time'] = device_time_full
                elif col == 'arrivalTime':  # Convert arrivalTime to CEST
                    arrival_date, arrival_time_full, _ = convert_to_cest(value)
                    structured_row['Arrival Date'] = arrival_date
                    structured_row['Arrival Time'] = arrival_time_full
...
def main():
    ...
    # Fetch data for the specific machine ID
    machine_data = fetch_machine_data(user_machine_id, access_token)

    if machine_data:
        logging.info(f"Data for Machine ID {user_machine_id}:")
        logging.info(machine_data)

        # Save raw data to Excel
        raw_output_file = "raw_machine_inactive_data.xlsx"
        pd.DataFrame(machine_data).to_excel(raw_output_file, index=False)
        logging.info(f"Raw data successfully saved to {raw_output_file}")

        # Process and save structured data
        structured_data = structure_data(machine_data)
        structured_output_file = "structured_machine_inactive_data.xlsx"
        structured_data.to_excel(structured_output_file, index=False)
        logging.info(f"Structured data successfully saved to {structured_output_file}")
    else:
        logging.info(f"No data found for Machine ID {user_machine_id}.")
