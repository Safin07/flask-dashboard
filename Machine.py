...
if all_machine_data:
    ...
    raw_output_file = "all_machine_data.xlsx"
    ...
    # Iterate over each row and column of the DataFrame
    for index, row in df.iterrows():
        new_row = {}
        for col, value in row.items():
            if isinstance(value, list):
                # If the value is a list, split it into separate columns
        ...
        # Append the new row to the structured DataFrame
        structured_data = structured_data.append(new_row, ignore_index=True)

    # Save the structured DataFrame to an Excel file
